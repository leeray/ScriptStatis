...
def api_init_Statictis(parent, filename, targetpath):
    ...
    while row:
        ...
        if data_match :
            ...
            obj_file[filename] = output_f
            result = []
            _args = ['time','ip','url','method','status','version','clientinfo','log_9','arg_sessionid', 'arg_guid', 'arg_vid','arg_type', 'arg_brand', 'arg_btype', 'arg_os', 'arg_wt', 'arg_ht', 'arg_imei', 'arg_imsi', 'arg_deviceid', 'arg_uuid','arg_brand', 'arg_ver', 'arg_time', 'arg_mac', 'arg_operator', 'arg_mobile','arg_uid','arg_os_ver','arg_network']
            for _arg in _args:
                result.append(jsonb.get(_arg,'#'))
