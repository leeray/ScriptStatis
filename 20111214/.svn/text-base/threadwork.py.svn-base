...
def getVideoInfo(vid):
    ...
    cat = ''
    try:
        path = 'http://{0}/openapi-wireless/videos/{1}?pid={2}'
        path = path.format('10.103.13.18', vid, 'b7b524c472e6a646')
        print path
        content = urllib2.urlopen(path, timeout=200).read()
        videoinfo = json.loads(content)
        result = videoinfo.get('results',{})
        cat = result.get('cats', 'NOCAT')
    except:
...
def readLog():
    ...
    ac_dict = {}
    for parent, dirnames, filenames  in os.walk('/v5/logs/20111214/SourceLog/'):
    ...
    output_f = codecs.open('/v5/logs/20111214/TargetLog/'+targetname+'.log', 'a', encoding=_encoding, errors=_errors)
...
def test():
    ...
    for i in range(500):
        wm.add_job( getVideoInfo, 'XMzE1MjU3MTM2')
